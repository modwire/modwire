...
class CoherenceReporter(InsightReporter):
    ...
    def collect(self, architecture_map: ArchitectureMap) -> CoherenceReport:
        ...
        for source_id in sorted(source_ids):
            has_incoming = architecture_map.code_map.incoming_dependencies(source_id).count() > 0
            has_outgoing = architecture_map.code_map.outgoing_dependencies(source_id).count() > 0
            if not has_incoming:
